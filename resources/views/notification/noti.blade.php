            ...
            @foreach ($res as $res)
                ...
                @if ($res->notification == 0)
                    <button class="h-2/5 mb-4 p-4 w-full grid content-center border-b-2 hover:bg-slate-200"
                        onClick="myFunction({{ $res->idUser }},{{ $res->idJobInfo }})">
                        <div class="flex gap-6">
                            <div class="w-16 h-16 grid content-center"><img
                                    src="{{ asset('profile/' . $res->idUser . '.jpg') }}" alt="You didn't Set Proflie"></div>
                            <div class="grid justify-items-start">
                                <p class="text-left font-semibold text-xl no-underline hover:underline">
                                    <strong> </strong> have sent the resume to you
                                </p>
                    </button>
                @endif
            @endforeach
