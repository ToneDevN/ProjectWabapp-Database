    ...
    public function updateenroll(Request $request)
    {
        ...
        if ($response) {
            if (isset($resume)) {
                ...
                $resumeFileName = auth()->user()->idUser . '' . $idjob . '' . $originalFileName;
        ...
        return redirect()->route('user.detail',['id'=>$idjob]);
    }
    ...
    public function submitResponse(Request $request)
    {
        ...
        if (isset($selectedOptions) && is_array($selectedOptions)) {
            foreach ($selectedOptions as $questionId => $answer) {
                ...
                $idJobInfo = $request->session()->get('jobId');

                $response = new responseJobInfo();
                $response->idUser = $userId;
                $response->idJobInfo = $idJobInfo;
            ...
            file_put_contents($resumeFilePath, $resumeFile);
                ...
                $idResponse = responseJobInfo::where('idUser', $userId)->where('idJobInfo', $idJobInfo)->first();
            
              

                // Store the question ID and answer in the responses array
                $responses[] = [
                    'idResponse' => $idResponse->idResponse, // Assuming 'id' is the primary key of response_job_infos
